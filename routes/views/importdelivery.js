var keystone = require('keystone'),
    Delivery = keystone.list('Delivery'),
    XLSX = require('xlsx'),
    async = require('async');
...
exports = module.exports = function (req, res) {
    ...
    view.on('post', function (next) {
        ...
        tasks.push(function (next) {
            var fields_map = {};
            var fields = keystone.list('Delivery').fields;
                ...
                if(rowNum > 0 ) {
                    (function (row, rowNum, action, updateId) {
                        dataUpdates.push(function (next) {
                            if(action === 'insert') {
                                Delivery.updateItem(new Delivery.model(), row, {}, function (err) {
                                    if(err) {
                                ...
                                Delivery.model.findById(updateId, function (err, model) {
                                    if(model) {
                                        Delivery.updateItem(model, row, {fields:update_fields}, function (err) {
                                            if(err) {
                                                locals.errors.push({row: rowNum + 1, detail: err.detail});
                                            }
                                            next();
