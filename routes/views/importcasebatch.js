var keystone = require('keystone'),
    CaseBatch = keystone.list('CaseBatch'),
    XLSX = require('xlsx'),
    async = require('async');
...
exports = module.exports = function (req, res) {
    ...
    // Set locals
    locals.section = 'importcasebatch';
    locals.errors = [];
    ...
    view.on('post', function (next) {
        ...
        tasks.push(function (next) {
            keystone.list('CaseBatch').model.find().exec(function (err, result) {
                casebatches = result;
                next(err);
            });
        });
        tasks.push(function (next) {
            var fields_map = {};
            var fields = keystone.list('CaseBatch').fields;
